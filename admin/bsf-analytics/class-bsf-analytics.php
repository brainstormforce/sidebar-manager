	...
	class BSF_Analytics {
			...
			Astra_Notices::add_notice(
				array(
					...
					'message'                    => sprintf(
						...
						sprintf( $notice_string . '<span dir="%2s"><a href="%3s" target="_blank" rel="noreferrer noopener">%4s</a><span>', esc_html( $this->get_product_name() ), $language_dir, esc_url( $this->usage_doc_link ), __( ' Know More.', 'sidebar-manager' ) ),
						add_query_arg(
							array(
		...
		public function render_settings_field_html() {
			...
			<label for="bsf-analytics-optin">
				...
				<?php
				esc_html_e( 'Allow Brainstorm Force products to track non-sensitive usage tracking data.', 'sidebar-manager' );
			...
			<?php
			echo wp_kses_post( sprintf( '<a href="%1s" target="_blank" rel="noreferrer noopener">%2s</a>', esc_url( $this->usage_doc_link ), __( 'Learn More.', 'sidebar-manager' ) ) );
			?>
			</fieldset>
			<?php
		}
